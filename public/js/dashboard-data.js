(function () {
  ...
  function renderOverview(d) {
    ...
    // Donut chart (3 segments: Completed, Ongoing, Pending)
    ...
    // Donut chart segments (3 segments: Pending, Ongoing, Completed)
    var circ = 408.4;
  ...
  function setupHouseFilter() {
    var sel = document.getElementById('houseFilter');              // mobile (scrollable section)
    var selM = document.getElementById('houseFilterMobile');       // mobile (below header)
    var selD = document.getElementById('houseFilterDesktop');      // desktop (in header)
    if (!sel && !selM && !selD) return;
    function apply(value) {
      _currentHouse = value;
      if (sel) sel.value = value;
      if (selM) selM.value = value;
      if (selD) selD.value = value;
    ...
    if (selD) selD.addEventListener('change', function () { apply(selD.value); });
  }
